...
export default function MasterAssetLibrary(){
 ...
 const rows=useMemo(()=>{
  ...
  return data.types
   .filter(t=>{
    ...
    return (!system||t.system_code===system)&&(!type||t.id===type)&&manufacturerMatch&&(!query||text.includes(query.toLowerCase()))
   })
   .map(t=>({
    ...t,
 ...
 },[data,system,type,manufacturer,query,manufacturerById])
  ...
  {data&&<>
   ...
   <div className="facility-panel filter-grid no-print">
    ...
    <Field label={lang==='ar'?'بحث':'Search'}>
     <input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Zamil / Carrier / FM-200 / Metasys / Chiller / UPS..."/>
    </Field>
   </div>
