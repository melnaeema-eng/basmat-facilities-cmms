...
export default function OwnerReports(){
 ...
 const filteredArchives=useMemo(()=>archiveFilter==='all'?archives:archives.filter(x=>(x.distribution_status||'final')===archiveFilter),[archives,archiveFilter])

 const refreshArchive=async()=>setArchives(await loadArchivedReports(contractId))
 const make=async()=>{
 ...
 const archive=async()=>{
  ...
  try{
   ...
   const snapshot={
    ...
    wo:report.wo.slice(0,200),ppm:report.ppm.slice(0,200),
 ...
 const openHistory=async run=>{
  try{
   setSelectedRun(run);setEvents(await loadReportEvents(run.id))
 ...
 return <section className="facility-module owner-report-center">
  <style>{`
   ...
   @media print{
    .app-sidebar,.bafm-topbar,.no-print,.page-head,.reports-hub-tabs,.reports-hub-head{display:none!important}
  ...
  `}</style>
  <div className="page-head"><div><h1>{ar?'مركز تقارير المالك':'Owner Reporting Center'}</h1><p>{ar?'تقارير أسبوعية وشهرية وسنوية مع سجل اعتماد وطباعة وإرسال.':'Weekly, monthly and annual reports with distribution history.'}</p></div></div>
  ...
  <div className="facility-panel no-print">
   ...
   <div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:12}}>
    ...
    {report&&<><button className="btn" onClick={()=>window.print()}>{ar?'معاينة / حفظ PDF':'Preview / Save PDF'}</button><button className="btn" disabled={loading} onClick={archive}>{ar?'اعتماد وأرشفة':'Finalize & Archive'}</button></>}
   </div>
  </div>
   ...
   <tbody>{filteredArchives.map(x=>{
    ...
    return <tr key={x.id}>
     ...
     <td><div className="or-actions">
      <button className="btn" onClick={()=>openHistory(x)}>{ar?'التفاصيل':'Details'}</button>
      <button className="btn" disabled={loading} onClick={()=>printArchived(x)}>{ar?'طباعة':'Print'}</button>
  ...
  {sendRun&&<div className="or-dialog-backdrop no-print" onClick={()=>setSendRun(null)}>
   <div className="or-dialog" onClick={e=>e.stopPropagation()}>
    <h2>{ar?'تسجيل إرسال التقرير':'Mark Report as Sent'}</h2>
    <p>{sendRun.report_number}</p>
    <label>{ar?'أرسل إلى (اسم/بريد/جهة)':'Sent to (name/email/entity)'}<input value={sendTo} onChange={e=>setSendTo(e.target.value)} placeholder={ar?'مثال: مدير المشروع - owner@example.com':'Example: Project Manager - owner@example.com'}/></label>
