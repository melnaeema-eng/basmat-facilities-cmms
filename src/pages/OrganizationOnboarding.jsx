...
export default function OrganizationOnboarding(){
 ...
 const edit=r=>{setEditing(r.id);setForm({name_ar:r.name_ar||'',name_en:r.name_en||'',organization_type:r.organization_type||'owner',registration_no:r.registration_no||'',vat_no:r.vat_no||'',email:r.email||'',phone:r.phone||'',city:r.city||'',address:r.address||''})}
 ...
 return <section className="facility-module">
  ...
  {(profile?.is_super_admin||editing)&&<form className="facility-panel" onSubmit={submit}><h2>{editing?t('orgEdit'):t('orgAdd')}</h2><div className="form-grid">
  ...
  <div className="security-check-list">{rows.map(r=><article className="facility-panel" key={r.id}><div className="page-head"><div><strong>{r.name}</strong><p className="muted">{r.code} · {t(r.organization_type||'private_company')}</p></div><button className="btn xs secondary" onClick={()=>edit(r)}>{t('orgEdit')}</button></div><div className="form-grid">
