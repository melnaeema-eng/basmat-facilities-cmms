   ...
   <header className="bafm-topbar">
    ...
    <div className="bafm-top-actions">
     ...
     <div className="bafm-user">
      <div className="bafm-avatar">{(profile?.full_name||user?.email||'U').slice(0,1).toUpperCase()}</div>
      <div><strong>{profile?.full_name||user?.email}</strong><small>{profile?.is_super_admin?'System Administrator':'Facilities User'}</small></div>
     </div>
