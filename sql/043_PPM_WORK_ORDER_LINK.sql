...
-- 4) Backfill already-assigned PPM jobs so the existing assignment immediately gets a WO number.
do $$
declare
  r record;
  new_wo uuid;
...
begin
  for r in
    select j.*, pl.contract_id
    from public.bf_ppm_jobs j
    left join public.bf_ppm_plans pl on pl.id=j.plan_id
    where j.work_order_id is null
      and j.status in ('assigned','in_progress','completed','approved','closed')
  loop
    new_wo := gen_random_uuid();
    wo_status := case
    ...
    insert into public.bf_work_orders(
      id,organization_id,client_id,site_id,contract_id,asset_id,
      work_order_number,title,description,priority,status,
      assigned_at,started_at,completed_at,closed_at,created_by
    ) values (
      new_wo,r.organization_id,r.client_id,r.site_id,r.contract_id,r.asset_id,
      'WO-'||to_char(now(),'YYYY')||'-'||lpad(nextval('public.bf4_wo_seq')::text,6,'0'),
      'Preventive Maintenance - '||r.job_number,
      'Generated automatically from PPM job '||r.job_number,
      'P3',wo_status,
      case when r.assigned_to is not null then coalesce(r.created_at,now()) else null end,
      r.started_at,r.completed_at,r.closed_at,
      coalesce(r.assigned_to,(select id from public.bf_profiles where status='active' order by created_at nulls last limit 1))
    );

    if r.assigned_to is not null then
      insert into public.bf_work_order_assignments(work_order_id,user_id,assigned_by)
      values(new_wo,r.assigned_to,r.assigned_to)
      on conflict do nothing;
    end if;

    update public.bf_ppm_jobs set work_order_id=new_wo where id=r.id;
  end loop;
end $$;
