...
export default function EnterpriseStructure(){
 const __teamUserPool=(()=>{
  const pools=[
   ...(typeof data!=='undefined'&&data ? [data.users,data.profiles,data.staff,data.directory,data.employees,data.portalUsers,data.portal_users] : []),
  ...
  for(const arr of pools)for(const x of arr){
   const id=x?.id||x?.user_id||x?.profile_id
   if(!id||seen.has(id))continue
   if(x?.status&&['inactive','archived','disabled'].includes(String(x.status).toLowerCase()))continue
   seen.add(id)
   out.push({...x,id})
  }
  return out
 })()
 const __teamUserLabel=(x)=>x?.full_name||x?.name_ar||x?.name_en||x?.name||x?.email||x?.username||x?.id||''
 ...
 return <section className="facility-module">
  ...
  <form className="facility-panel" onSubmit={submitMember}><h2>{t('esAddMember')}</h2><div className="form-grid">
   ...
   <label>{t('esUser')}<select required value={member.user_id} onChange={e=>setMember(v=>({...v,user_id:e.target.value}))}><option value="">—</option>{__teamUserPool.map(x=><option key={x.id} value={x.id}>{x.full_name||x.email}</option>)}
     {__teamUserPool.map(x=><option key={x.id} value={x.id}>{__teamUserLabel(x)}</option>)}</select></label>
