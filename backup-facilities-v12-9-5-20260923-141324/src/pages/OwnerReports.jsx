...
import {loadOwnerReportDirectory,loadBranding,saveBranding,loadOwnerReportData,archiveOwnerReport,loadArchivedReports} from '../lib/ownerReports'
...
export default function OwnerReports(){
 ...
 useEffect(()=>{loadOwnerReportDirectory().then(setDirectory).catch(e=>setError(e.message))},[])
 useEffect(()=>{setPeriod(periodFor(reportType));setReport(null)},[reportType])
 useEffect(()=>{
  if(!contractId){setBranding(null);setArchives([]);return}
  Promise.all([loadBranding(contractId),loadArchivedReports(contractId)]).then(([b,a])=>{setBranding(b);setArchives(a)}).catch(e=>setError(e.message))
 },[contractId])
 ...
 const archive=async()=>{
  ...
  try{
   ...
   const r=await archiveOwnerReport({
    ...
    period_start:start,period_end:end,report_number:reportNumber,revision:'00',status:'final',
    title_ar:reportTitle(true,reportType),title_en:reportTitle(false,reportType),
    snapshot:{metrics:report.metrics,woStatus:report.woStatus,ppmStatus:report.ppmStatus,woPriority:report.woPriority,labels:report.labels},
    branding_snapshot:currentBrand
   })
   setSaved((ar?'تم أرشفة التقرير: ':'Archived report: ')+r.report_number)
   setArchives(await loadArchivedReports(contractId))
  }catch(e){setError(e.message)}finally{setLoading(false)}
 }
 ...
 return <section className="facility-module owner-report-center">
  <style>{`
   ...
   .or-table{width:100%;border-collapse:collapse}.or-table th,.or-table td{font-size:10px;padding:7px;border-bottom:1px solid #edf1f5;text-align:start}
   ...
   @media print{
    .app-sidebar,.bafm-topbar,.no-print,.page-head{display:none!important}
    .bafm-main,.bafm-content{margin:0!important;padding:0!important}
    .facility-panel{box-shadow:none!important}.owner-report-center{font-size:10px}
    .or-cover{page-break-after:always;border:none;min-height:95vh}.or-section{break-inside:avoid}
    @page{size:A4;margin:12mm}
   }
  `}</style>
  <div className="page-head"><div><h1>{ar?'مركز تقارير المالك':'Owner Reporting Center'}</h1><p>{ar?'تقارير أسبوعية وشهرية وسنوية بهوية المالك والمقاول.':'Professional weekly, monthly and annual owner reports.'}</p></div></div>
  ...
  <div className="facility-panel no-print">
   <div className="or-controls">
    <label>{ar?'العقد':'Contract'}<select value={contractId} onChange={e=>{setContractId(e.target.value);setReport(null)}}><option value="">{ar?'اختر العقد':'Select contract'}</option>{directory?.contracts.map(c=><option key={c.id} value={c.id}>{c.contract_number||c.number||c.code||c.id}</option>)}</select></label>
   ...
   <div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:12}}>
    ...
    {report&&<><button className="btn" onClick={()=>window.print()}>{ar?'طباعة / حفظ PDF':'Print / Save PDF'}</button><button className="btn" disabled={loading} onClick={archive}>{ar?'اعتماد وأرشفة':'Finalize & Archive'}</button></>}
   </div>
  </div>
  ...
  {report&&<ReportDocument report={report} brand={currentBrand} reportType={reportType} start={start} end={end} ar={ar}/>}

  {!!archives.length&&<div className="facility-panel no-print" style={{marginTop:16}}>
   <h2>{ar?'أرشيف التقارير':'Report Archive'}</h2>
   <table className="or-table"><thead><tr><th>{ar?'رقم التقرير':'Report No.'}</th><th>{ar?'النوع':'Type'}</th><th>{ar?'الفترة':'Period'}</th><th>{ar?'المراجعة':'Rev'}</th><th>{ar?'الحالة':'Status'}</th></tr></thead>
   <tbody>{archives.map(x=><tr key={x.id}><td>{x.report_number}</td><td>{x.report_type}</td><td>{x.period_start} — {x.period_end}</td><td>{x.revision}</td><td>{x.status}</td></tr>)}</tbody></table>
  </div>}
 </section>
}
...
function ReportDocument({report,brand,reportType,start,end,ar}){
 const m=report.metrics
 const summary=ar
  ?`خلال الفترة من ${start} إلى ${end} تم تسجيل ${m.totalWO} أمر عمل، أُغلق منها ${m.completedWO} بنسبة إنجاز ${m.woClosure}%. بلغت نسبة الالتزام بالصيانة الوقائية ${m.ppmCompliance}%، مع ${m.overduePPM} مهمة PPM متأخرة و${m.overdueWO} أمر عمل متأخر.`
  :`During ${start} to ${end}, ${m.totalWO} work orders were recorded and ${m.completedWO} were completed, giving a ${m.woClosure}% closure rate. PPM compliance was ${m.ppmCompliance}%, with ${m.overduePPM} overdue PPM jobs and ${m.overdueWO} overdue work orders.`
 ...
 return <div className="owner-report-document">
  <div className="or-cover" style={{borderTop:`8px solid ${brand.primary_color||'#0b2b4b'}`}}>
   ...
   <div className="or-meta">
    ...
    <div><span>{ar?'تاريخ الإصدار':'Issue date'}</span><b>{new Date().toLocaleDateString()}</b></div>
  ...
  <div className="or-section"><h2>{ar?'الملخص التنفيذي':'Executive Summary'}</h2><div className="facility-panel"><p>{summary}</p></div></div>
  <div className="or-section"><h2>{ar?'مؤشرات الأداء الرئيسية':'Key Performance Indicators'}</h2>
   <div className="or-kpis">
    <Kpi l={ar?'أوامر العمل':'Work Orders'} v={m.totalWO}/><Kpi l={ar?'نسبة الإغلاق':'WO Closure'} v={m.woClosure+'%'} tone={pctTone(m.woClosure)}/>
    <Kpi l={ar?'PPM المنفذة':'PPM Completed'} v={`${m.completedPPM}/${m.totalPPM}`}/><Kpi l={ar?'التزام PPM':'PPM Compliance'} v={m.ppmCompliance+'%'} tone={pctTone(m.ppmCompliance)}/>
    <Kpi l={ar?'WO متأخرة':'Overdue WO'} v={m.overdueWO}/><Kpi l={ar?'PPM متأخرة':'Overdue PPM'} v={m.overduePPM}/>
    <Kpi l={ar?'أصول حرجة':'Critical Assets'} v={m.criticalAssets}/><Kpi l="SLA" v={m.slaRate==null?'N/A':m.slaRate+'%'} tone={m.slaRate==null?null:pctTone(m.slaRate)}/>
   </div>
  </div>

  <div className="or-section"><h2>{ar?'توزيع أوامر العمل':'Work Order Distribution'}</h2><div className="facility-panel or-chart">
  ...
  <div className="or-section"><h2>{ar?'الأعمال التصحيحية':'Corrective Maintenance'}</h2><ReportTable rows={report.wo.slice(0,30)} cols={[
  ...
  <div className="or-section"><h2>{ar?'الصيانة الوقائية':'Preventive Maintenance'}</h2><ReportTable rows={report.ppm.slice(0,30)} cols={[
   ['job_number',ar?'رقم المهمة':'Job No.'],['title',ar?'العنوان':'Title'],['status',ar?'الحالة':'Status'],['due_date',ar?'الاستحقاق':'Due']
  ]}/></div>

  <div className="or-section"><h2>{ar?'الأصول والضمان':'Assets & Warranty'}</h2>
   <div className="or-kpis"><Kpi l={ar?'إجمالي الأصول':'Total assets'} v={m.assets}/><Kpi l={ar?'الأصول الحرجة':'Critical assets'} v={m.criticalAssets}/><Kpi l={ar?'ضمان ينتهي خلال الفترة':'Warranty ending'} v={m.warrantyEnding}/></div>
  </div>

  <div className="or-section"><h2>{ar?'المخاطر وخطة الفترة القادمة':'Risks & Next Period Plan'}</h2><div className="facility-panel">
   <p>{ar?'يتم التركيز على إغلاق الأعمال المتأخرة، معالجة الأعطال الحرجة، وتحسين الالتزام بخطة الصيانة الوقائية، مع متابعة قطع الغيار والأصول ذات الأولوية العالية.':'Focus will remain on closing overdue work, resolving critical failures, improving PPM compliance, and monitoring critical assets and required spares.'}</p>
  </div></div>
