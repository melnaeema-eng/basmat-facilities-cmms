...
export async function loadMaintenanceCoverage(){
 const {data,error}=await supabase
  .from('bf_maintenance_library_coverage')
  .select('*')
  .order('domain')
  .order('name_en')
...
export async function loadMaintenanceQuality(){
 const {data,error}=await supabase
  .from('bf_maintenance_source_quality')
  .select('*')
  .order('domain')
 if(error)throw error
 return data||[]
...
export async function rebuildFacilityPackage(jobId){
 const {data,error}=await supabase.rpc('bf_build_facility_execution_package',{p_job:jobId})
 if(error)throw error
 return data
}

export async function rebuildMedicalPackage(workOrderId){
 const {data,error}=await supabase.rpc('bf_build_medical_execution_package',{p_work_order:workOrderId})
 if(error)throw error
 return data
}
